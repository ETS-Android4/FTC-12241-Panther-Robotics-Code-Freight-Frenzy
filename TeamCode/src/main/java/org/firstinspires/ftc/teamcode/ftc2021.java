    ...
    public DistanceSensor steveO;
    //public DcMotor armVertical = null;
    //public DcMotor armHorizontal = null;
    //public DcMotor counter = null;
    //public Servo grabber1 = null;
    //public Servo grabber2 = null;
    Rev2mDistanceSensor sensorTimeOfFlight = (Rev2mDistanceSensor)steveO;
    ...
    private static final double TICK_FACTOR = (35.0 / 45.0) * (560 / Math.PI / 4);
    ...
    public void init(HardwareMap ahwMap, Telemetry telem) {
        ...
        steveO = hwMap.get(DistanceSensor.class, "steve");
...
//        setCapArm(1);
    ...
    /*public void distance(){
        double value = steveO.getDistance(DistanceUnit.INCH);
        if (value >23{
    ...
    /*public void barcodeDistanceRed(){
        steveO.getDistance(DistanceUnit.INCH);
        if ( value <= 26) {
            move(15, 0, 0, 0.5, 3);
            move(0, 0, 80, 0.3, 3);
            move(25, 0, 0, 0.5, 3);
    ...
    private int move(int fl_delta, int fr_delta, int bl_delta, int br_delta, double power, double maxTime) {
        ...
        setDrivePower(power * Math.abs(fl_delta), power * Math.abs(fr_delta),
                power * Math.abs(bl_delta), power * Math.abs(br_delta));
    ...
    public void runToPosition(boolean usePos) {
        ...
        if (usePos) {
            ...
            //rmVertical.setMode(DcMotor.RunMode.RUN_TO_POSITION);
            //armHorizontal.setMode(DcMotor.RunMode.RUN_TO_POSITION);
            //counter.setMode(DcMotor.RunMode.RUN_TO_POSITION);
            ...
            //counter.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        }
    }
    ...
    public void move(double fwd, double sideways, double degrees, double power, double maxTime) {
        ...
        double delta_t = (TICK_FACTOR * Math.PI * 28 * degrees) / 360.0; // turning radius; distance of opposing wheels
    ...
    public void turn(double degree, double power, double maxTime) {
        double distance_inInch = Math.PI * 28 * degree / 360.0; // turning radius; distance of opposing wheels
        int delta;
